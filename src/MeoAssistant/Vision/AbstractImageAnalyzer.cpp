...
bool asst::AbstractImageAnalyzer::save_img(const std::string& dirname, const std::string& filename)
{
    ...
    if (filename.empty()) {
        ...
        full_path = dirname + stem + "_raw.png";
    }
    else {
        full_path = dirname + filename + ".png";
    }
    Log.trace("Save image", full_path);
    bool ret = asst::imwrite(full_path, m_image(make_rect<cv::Rect>(m_roi)));
    //
    // #ifdef ASST_DEBUG
